...
数据采用双重限额：每次保存时立即把每个账号的对话窗口裁剪到最近 100 个，前端历史每页只读取 10 个标题，并按需读取当前对话正文。后台还会在启动时及每 30 天重新检查一次，并同步清理已淘汰窗口的已完成任务、孤立记录和每个账号超过 30 条的旧任务。SQLite 文件至少达到 1 MiB 且空闲页超过 25% 时才执行 `VACUUM`，避免低配机器频繁压缩数据库。
