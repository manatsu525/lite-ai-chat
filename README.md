...
知乎、百度系、抖音、头条、小红书等站点可能要求登录/验证码，或对 VPS 自动访问返回 403/429。项目不会为此在低内存机器上常驻 Chromium：提示词和工具说明会优先引导模型选择可公开读取的来源；模型仍选择已知强反爬域名时，后端会改抓同一轮搜索中尚未使用的公开候选，并在过程面板明确记录来源替换。若替代来源仍失败，则复用对应搜索结果的标题与索引摘要并标记为“非网页全文”。没有可用摘要时会如实返回源站限制，不会伪造正文。
